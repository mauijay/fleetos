    ...
    <section class="section"><div class="mapping-list">
        ...
        <?php foreach ($workflows as $workflow): ?>
            <article class="mapping-card tone-<?= $workflow['workflow_status'] === 'completed' ? 'success' : 'warning' ?>">
                <div class="mapping-card-main"><div><div class="vehicle-status-row"><span class="status-badge tone-info"><?= esc(ucfirst($workflow['movement_type'])) ?></span><span><?= esc(ucwords(str_replace('_', ' ', $workflow['workflow_status']))) ?></span></div><h3><?= esc((string) ($workflow['fleet_code'] ?? $workflow['display_name'] ?? 'Vehicle')) ?></h3><p><?= esc((string) ($workflow['airport_code'] ?? 'HNL')) ?> · <?= esc((string) $workflow['scheduled_at']) ?></p><?php if (($workflow['garage_attention'] ?? null) !== null): ?><p class="tone-danger"><strong>Wrong airport garage</strong> · Recovery / relocation required</p><?php endif; ?></div><dl class="issue-facts"><div><dt>Garage</dt><dd><?= esc((string) ($workflow['garage_line'] ?? 'Not recorded')) ?></dd></div><div><dt>Position</dt><dd><?= esc((string) ($workflow['position_line'] ?? 'Not recorded')) ?></dd></div></dl></div>

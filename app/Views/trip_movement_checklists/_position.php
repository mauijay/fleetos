...
<section class="section position-panel" aria-labelledby="position-heading">
    <div class="section-heading"><div><p class="eyebrow">Current vehicle position</p><h2 id="position-heading"><?= $isRented ? 'Rented' : esc($locationLabels[$locationClass] ?? ucwords(str_replace('_', ' ', $locationClass))) ?></h2></div><a class="action-link" href="/fleet/vehicles/<?= (int) $checklist['fleet_vehicle_id'] ?>/positioning-plan">Positioning plan</a></div>
    <?php if ($isRented): ?><p class="position-detail">Guest handoff recorded<?= trim((string) ($currentLocation['location_detail'] ?? '')) === '' ? '' : ' at ' . esc((string) $currentLocation['location_detail']) ?>.</p>
    <?php else: ?><p class="position-detail"><?= trim((string) ($currentLocation['location_detail'] ?? '')) === '' ? 'No additional location detail.' : esc((string) $currentLocation['location_detail']) ?><?php if (($currentLocation['observed_at'] ?? null) !== null): ?> · confirmed <?= esc(date('M j, g:i A', strtotime((string) $currentLocation['observed_at']))) ?><?php endif; ?></p><?php endif; ?>

    <?php if (! $isRented && ! $showPositionForm): ?><a class="primary-action position-action" href="/operations/checklists/<?= (int) $checklist['id'] ?>?action=position#position-entry">Record vehicle position</a><?php endif; ?>
    <?php if (! $isRented && $nextHnlPickupHref !== null): ?><a class="secondary-action position-action" href="<?= esc((string) $nextHnlPickupHref, 'attr') ?>#handoff-entry">Stage at HNL for next pickup</a><?php endif; ?>
    <?php if (! $isRented && $showPositionForm): ?>
        <form id="position-entry" class="issue-filters position-form" action="/operations/checklists/<?= (int) $checklist['id'] ?>/vehicle-position" method="post">
            ...
            <label>Actual location<select id="position-location-class" name="location_class" required><?php foreach (['home', 'waikiki_hotel', 'other_delivery', 'airport_hnl'] as $location): ?><?php if (($checklist['movement_type'] ?? '') === 'pickup' && $location === 'airport_hnl') continue; ?><option value="<?= esc($location, 'attr') ?>" <?= $selectedLocation === $location ? 'selected' : '' ?>><?= esc($locationLabels[$location]) ?></option><?php endforeach; ?></select></label>
            ...
            <?php if (($checklist['movement_type'] ?? '') !== 'pickup'): ?><fieldset class="hnl-parking-fields" data-hnl-parking data-location-select="position-location-class"><legend>HNL parking</legend><label>Row<select name="airport_parking_row" data-hnl-row><option value="">Choose row</option><?php foreach ($hnlGarages as $code => $garage): ?><?php foreach ($garage['rows'] as $row): ?><option value="<?= esc($row, 'attr') ?>" data-garage="<?= esc($code, 'attr') ?>" <?= $selectedRow === $row ? 'selected' : '' ?>><?= esc($row) ?></option><?php endforeach; ?><?php endforeach; ?></select></label><label>Garage<select name="airport_garage_code" data-hnl-garage><option value="">Derived from row</option><?php foreach ($hnlGarages as $code => $garage): ?><option value="<?= esc($code, 'attr') ?>" data-max-level="<?= (int) $garage['levels'] ?>" <?= $selectedGarage === $code ? 'selected' : '' ?>><?= esc($garage['name']) ?> · <?= esc($garage['color']) ?></option><?php endforeach; ?></select></label><label>Level<select name="airport_parking_level" data-hnl-level><option value="">Choose level</option><?php for ($level = 1; $level <= 8; $level++): ?><option value="<?= $level ?>" <?= $selectedLevel === (string) $level ? 'selected' : '' ?>><?= $level ?></option><?php endfor; ?></select></label></fieldset><?php endif; ?>
    ...
    <?php elseif (($checklist['movement_type'] ?? '') === 'pickup' && ! $isRented): ?><p class="muted">Use the pickup staging workflow for an actual HNL guest staging position.</p><?php endif; ?>
</section>

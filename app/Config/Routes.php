...
$routes->group('', ['filter' => 'session'], static function (RouteCollection $routes): void {
    ...
    $routes->group('fleet/vehicles', ['filter' => 'permission:admin.access'], static function (RouteCollection $routes): void {
        ...
        $routes->post('(:num)/current-position', 'VehicleCurrentState::recordPosition/$1', ['filter' => 'csrf']);
        $routes->post('(:num)/current-readiness', 'VehicleCurrentState::recordReadiness/$1', ['filter' => 'csrf']);
    });
    ...
    $routes->post('operations/checklists/(:num)/photos-complete', 'TripMovementChecklists::completePhotos/$1', ['filter' => ['permission:admin.access', 'csrf']]);
    $routes->post('operations/checklists/(:num)/photos-undo', 'TripMovementChecklists::undoPhotos/$1', ['filter' => ['permission:admin.access', 'csrf']]);
    $routes->post('operations/checklists/(:num)/charging-adapter-present', 'TripMovementChecklists::confirmChargingAdapter/$1', ['filter' => ['permission:admin.access', 'csrf']]);
    $routes->post('operations/checklists/(:num)/charging-adapter-undo', 'TripMovementChecklists::undoChargingAdapter/$1', ['filter' => ['permission:admin.access', 'csrf']]);

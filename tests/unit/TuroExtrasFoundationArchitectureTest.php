...
final class TuroExtrasFoundationArchitectureTest extends CIUnitTestCase
{
    ...
    public function testViewProvidesFoundationWorkflowWithoutPerformanceReport(): void
    {
        ...
        $this->assertMatchesRegularExpression('/\.extra-map-form \{.*?display: flex;.*?flex-wrap: wrap;/s', $css);
        $this->assertMatchesRegularExpression('/\.extra-map-form > label \{.*?flex: 1 1 12rem;.*?min-width: 0;/s', $css);
        $this->assertMatchesRegularExpression('/\.extra-map-form select \{.*?min-width: 0;.*?width: 100%;/s', $css);
        $this->assertMatchesRegularExpression('/\.extra-map-form > button \{.*?max-width: 100%;.*?white-space: normal;/s', $css);
